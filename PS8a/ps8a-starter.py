...
def load_data(data_dir, top_n=10):
    ...
    print(f'    Size of target_top_n list = {len(target_top_n)}')
    ...
    for i in  range(unsorted_name_list.shape[0]):
        ...
        if (unsorted_name_list[i,0] in names_top_n):
            index_list.append(i)
        # index_list.extend(np.ndarray.tolist(np.where(unsorted_name_list == name)[0]))
    ...
    data_top_n = [row.flatten() for row in data_top_n]
    print(f'    Size of data_top_n list = {len(data_top_n)}')
...
def perform_pca(data_train, data_test, data_noneface, n_components, plot_PCA=False):
    ...
    data_test_centered = scaler.fit_transform(data_test_centered)
    print(f'Data Test Shape : {data_test_centered.shape}')

    data_noneface_centered = np.array([row.flatten() for row in data_noneface])
    data_noneface_centered = scaler.fit_transform(data_noneface_centered)
...
def plot_eigenfaces(pca):
    ...
    for i in range(n_row * n_col):
        ...
        axes[i%4][i//4].imshow(eigenfaces[i].reshape(IMG_SHAPE), cmap="gray")
    plt.show()
