    ...
    for i in range(len(pick[idx])):
        if idx == 3:    #to retain the red points on the center image
            for j in range(len(pick[1])):
                ...
                dst = cv2.putText(dst, str(j), (pick[1][j][0]+10, pick[1][j][1]-10),
...
# Enter Dataset number to use: 0 for wall dataset, 1 for door data, 2 for house data, 3 for pittsburgh data
